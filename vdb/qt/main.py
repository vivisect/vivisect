...
class VdbWindow(vq_app.VQMainCmdWindow):
    ...
    def hotkeys(self, db, line):
        ...
        for targname in targets:
            db.vprint('%s: %s' %
                      (targname.ljust(20), lookup.get(targname, '')))
    ...
    def vqInitDockWidgetClasses(self):
        self.vqAddDockWidgetClass(
            vdb.qt.memory.VdbMemoryWindow, args=(self._db, self._db_t))
        self.vqAddDockWidgetClass(
            vtrace.qt.VQMemoryMapView, args=(self._db_t, self))
        self.vqAddDockWidgetClass(
            vtrace.qt.VQFileDescView, args=(self._db_t, self))
        self.vqAddDockWidgetClass(
            vdb.qt.threads.VdbThreadsWindow, args=(self._db, self._db_t,))
        self.vqAddDockWidgetClass(vqt.qpython.VQPythonView, args=(
            self._db.getExpressionLocals(),))

        self.vqAddDockWidgetClass(
            vdb.qt.registers.VdbRegistersWindow, args=(self._db, self._db_t))
        self.vqAddDockWidgetClass(
            vdb.qt.memwrite.VdbMemWriteWindow, args=(self._db, self._db_t,))
    ...
    def menuViewThreads(self):
        self.vqBuildDockWidget(
            'VdbThreadsWindow', area=QtCore.Qt.RightDockWidgetArea)

    @vq_hotkeys.hotkey('vdb:view:registers')
    def menuViewRegisters(self):
        self.vqBuildDockWidget('VdbRegistersWindow',
