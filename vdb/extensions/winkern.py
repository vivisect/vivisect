...
def bugcheck(db, line):
    ...
    if symname == 'nt.KeBugCheckEx':
        ...
        if sig == KERNEL_MODE_EXCEPTION_NOT_HANDLED:
            retaddr, stacksig, excode, exaddr, extrap = trace.readMemoryFormat(
                sp, '<5I')
            ...
            db.vprint('')
            db.vprint('Registers:')
            db.vprint('eax=0x%.8x ebx=0x%.8x ecx=0x%.8x edx=0x%.8x' %
                      (trap.Eax, trap.Ebx, trap.Ecx, trap.Edx))
            db.vprint('esi=0x%.8x edi=0x%.8x ebp=0x%.8x esp=0x????????' %
                      (trap.Esi, trap.Edi, trap.Ebp))
...
def vads(db, line):
    ...
    def sortVas(k):
        x, y = k
        xstart = x[1].StartingVpn
        ystart = y[1].StartingVpn
        return (xstart > ystart) - (xstart < ystart)
    vads.sort(key=sortVas)
...
def walkVads(db, trace, vadroot):
    ...
    while todo:
        ...
        if l:
            todo.append((l, trace.getStruct('nt.MMVAD_SHORT', va=l)))
        if r:
            todo.append((r, trace.getStruct('nt.MMVAD_SHORT', va=r)))
...
def showeproc(db, trace, opts, eprocva, eproc):
    ...
    db.vprint('eprocess: 0x%.8x vad: 0x%.8x peb: 0x%.8x objs: 0x%.8x %s' %
              (eprocva, vadroot, peb, objtable, pname))
...
def eproc(db, line):
    ...
    parser.add_option('--threads', dest='dothreads',
                      default=False, action='store_true')
    parser.add_option('--handles', dest='dohandles',
                      default=False, action='store_true')
    parser.add_option('--objects', dest='doobjects',
                      default=False, action='store_true')
