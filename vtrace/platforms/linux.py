...
class LinuxMixin(v_posix.PtraceMixin, v_posix.PosixMixin):
    ...
    def platformReadMemory(self, address, size):
        ...
        if x != size:
            ...
            raise Exception(
                "reading from invalid memory %s (%d returned)" % (hex(address), x))
...
class Linuxi386Trace(
    ...
    def platformAllocateMemory(self, size, perms=e_mem.MM_RWX, suggestaddr=0):
        ...
        mma = struct.pack("<6L", suggestaddr, size, realperm,
...
class LinuxAmd64Trace(vtrace.Trace, LinuxMixin, v_amd64.Amd64Mixin, v_posix.ElfMixin, v_base.TracerBase):
...
class LinuxArmTrace(vtrace.Trace, LinuxMixin, v_arm.ArmMixin, v_posix.ElfMixin, v_base.TracerBase):
