...
def analyzeJmp(amod, emu, op, starteip):
    ...
    if test:
        makeSwitch(emu.vw, starteip, ctx['offarraybase'], ctx['indiroffbase'])
...
def testSwitch(vw, op, vajmp, emu=None):
    ...
    if not (isinstance(arrayoper, e_i386.i386SibOper) and arrayoper.scale == 4):
        vw.verbprint("arrayoper is not an i386SibOper of size 4: %s" %
                     repr(baseoper))
        return False, None
    ao_reg = arrayoper.reg & e_i386.RMETA_NMASK
    if ao_reg != baseoper.reg:
        vw.verbprint("arrayoper.reg != baseoper.reg: %s != %s" %
                     (ao_reg, baseoper.reg))
        return False, None
    ...
    if offarrayfirst+imagebase != vajmp+2:
        vw.verbprint("first ref is not the va after the jmp: %x != %x" %
                     (offarrayfirst+imagebase, vajmp+2))

    indiroffbase = None
    # now check for the byte array before that
    # this one is optional. first two are not.
    backthree = vw.getLocation(backtwo[0]-1)
    if backthree != None:
        op = vw.parseOpcode(backthree[0])
        if op.mnem == 'movzx' and isinstance(op.opers[1], e_i386.i386SibOper) and \
                op.opers[1].scale == 1:
            vw.verbprint(
                "this is a double deref (hitting a byte array offset into the offset-array)")
