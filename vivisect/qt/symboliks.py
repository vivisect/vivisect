...
class VivSymbolikFuncPane(e_q_memory.EnviNavMixin, vq_save.SaveableWidget, QWidget):
    ...
    def __init__(self, vw, parent=None):
        ...
        navbox = HBox(fvalabel, self.exprtext, inccblabel,
                      self.constraintext, looplabel, self.loop_count, self.alleffs)
            ...
            if cexpr:
                ...
                if ccb is not None and ccb in self.vw.getFunctionBlocks(self.fva):
                    ...
                    codepaths = viv_graph.getCodePathsThru(
                        codegraph, ccb[0], loopcnt=loopcnt)
                    paths = self.symctx.getSymbolikPaths(
                        self.fva, paths=codepaths, graph=codegraph, maxpath=100)

            if codepaths is None:
                loopcnt = self.loop_count.value()
                paths = self.symctx.walkSymbolikPaths(
                    self.fva, maxpath=100, loopcnt=loopcnt)
