...
# A type to hold a known-derefable pointer that is of appropriate length for arch
LOC_POINTER = 4
LOC_OP = 5  # An opcode
...
# A sequence of bytes which is a pad (string nulls, MS hotpatch... (char is tinfo)
LOC_PAD = 10
LOC_MAX = 11
...
# Relocation types
# VA contains a pointer to a va (and is assumed fixed up by parser)
RTYPE_BASERELOC = 0
RTYPE_BASEOFF = 1  # Add Base and Offset to a pointer at a memory location
# Like BASEOFF, but treated as a Pointer, not part of an instruction/etc.
RTYPE_BASEPTR = 2
