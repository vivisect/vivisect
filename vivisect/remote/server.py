...
def connectToServer(hostname):
    ...
    server = cobra.CobraProxy("cobra://%s:%d/VivServer" %
                              (hostname, viv_port), msgpack=True)
    version = server.getServerVersion()
    if version != server_version:
        raise Exception('Incompatible Server: his ver: %d our ver: %d' % (
...
if __name__ == '__main__':
    ...
    parser = optparse.OptionParser(
        usage='python -m vivisect.remote.server <vivdir>')
