...
class PpcEmbeddedWorkspaceEmulator(v_i_emulator.WorkspaceEmulator, e_ppc.PpcEmbeddedEmulator):

    def __init__(self, vw, logwrite=False, logread=False):
        self.taintregs = [ x for x in range(13) ]
        self.taintregs.append(e_ppc.REG_LR)

        e_ppc.PpcEmbeddedEmulator.__init__(self)
...
class PpcVleWorkspaceEmulator(PpcEmbeddedWorkspaceEmulator):
    def parseOpcode(self, va, arch=envi.ARCH_DEFAULT):
	# We can make an opcode *faster* with the workspace because of
	# getByteDef etc... use it.
	op = self.opcache.get(va)
...
class PpcServerWorkspaceEmulator(PpcEmbeddedWorkspaceEmulator):
    def parseOpcode(self, va, arch=envi.ARCH_DEFAULT):
	# We can make an opcode *faster* with the workspace because of
	# getByteDef etc... use it.
	op = self.opcache.get(va)
	if op == None:
	    op = self.vw.parseOpcode(va, arch=envi.ARCH_PPC_S)
	    self.opcache[va] = op
