...
# XXX(rakuyo): maybe rip this out and replace it with json schema or something?
...
class EnviConfig:
    ...
    def getConfigPaths(self):
        ...
        while todo:
            ...
            if cfgkeys:
                pathstr = '.'.join(path) + "."
                newpaths = [(CONFIG_ENTRY, "%s%s" % (pathstr, key),
                             "%s" % (config[key])) for key in cfgkeys]
    ...
    def reprConfigPaths(self):
        ...
        reprs = ['%s = %s' % (ckey, cval) for ctype, ckey,
                 cval in configpaths if ctype == CONFIG_ENTRY]
        out.append("\n    ".join(reprs))
        out.append("\n")

        out.append("\nValid Config Paths:\n    ")
        reprs = [ckey for ctype, ckey,
                 cval in configpaths if ctype == CONFIG_PATH]
        out.append("\n    ".join(reprs))
        out.append("\n")
        return ''.join(out)
