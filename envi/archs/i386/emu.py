...
class BFastCall(ThisCall):
    arg_def = [(CC_REG, REG_EAX), (CC_REG, REG_EDX), (CC_REG, REG_ECX), (CC_STACK_INF, 4)]
...
class IntelEmulator(i386RegisterContext, envi.Emulator):
    ...
    def writeMemValue(self, addr, value, size):
        ...
        if size == 1:
            ...
            mask = 0xffffffffffffffff
            nums = (value & mask, (value >> 64) & mask,
                    (value >> 128) & mask, (value >> 192) & mask)
    ...
    def cond_le(self):
        return (self.getFlag(EFLAGS_SF) != self.getFlag(EFLAGS_OF) or (self.getFlag(EFLAGS_ZF) == 1))
    ...
    def logicalAnd(self, op):
        ...
        # AF is undefined, but it seems like it is zeroed
    ...
    def i_xor(self, op):
        ...
        self.setFlag(EFLAGS_ZF, not ret)
        self.setFlag(EFLAGS_PF, e_bits.is_parity_byte(ret))
        # Undefined but actually cleared on amd64 X2
        self.setFlag(EFLAGS_AF, False)
