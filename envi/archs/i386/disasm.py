...
iflag_lookup = {
    opcode86.INS_RET: e_const.IF_NOFALL | e_const.IF_RET,
    opcode86.INS_CALL: e_const.IF_CALL,
    opcode86.INS_HALT: e_const.IF_NOFALL,
    opcode86.INS_DEBUG: e_const.IF_NOFALL,
    opcode86.INS_CALLCC: e_const.IF_CALL | e_const.IF_COND,
    opcode86.INS_BRANCH: e_const.IF_NOFALL | e_const.IF_BRANCH,
    opcode86.INS_BRANCHCC: e_const.IF_BRANCH | e_const.IF_COND,
    opcode86.INS_MOVCC: e_const.IF_COND,
    opcode86.INS_XCHGCC: e_const.IF_COND,
}
...
class i386SibOper(envi.DerefOper):
    ...
    def render(self, mcanv, op, idx):
        ...
        if self.index is not None:
            ...
            rname = self._dis_regctx.getRegisterName(self.index & e_const.RMETA_NMASK)
            mcanv.addNameText(name, name=rname, typename="registers")
            if self.scale != 1:
                mcanv.addText(" * ")
...
class i386Disasm:
    ...
    def __init__(self, mode=MODE_32):
        ...
        # TODO(rakuyo): maybe change this to a dict? It'd be about as fast and save space
        # (but would induce an extra call to __hash__, sooo...maybe not? debate)
        ...
        self.ROFFSETSIMD = self._dis_regctx.getRegisterIndex("xmm0")
    ...
    def extended_parse_modrm(self, bytez, offset, opersize, regbase=0, prefixes=0):
        ...
        if mod == 3:  # Easy one, just a reg
            ...
            if rm == 5:
                ...
                oper = i386SibOper(opersize, reg=base, imm=imm,
                                   index=index, scale=scale_lookup[scale])
                return (size, oper)
            ...
            if rm == 4:
                ...
                oper = i386SibOper(opersize, reg=base, index=index,
                                   scale=scale_lookup[scale], disp=disp)
                return (size, oper)
            else:
            ...
            if rm == 4:
                ...
                oper = i386SibOper(
                    opersize, reg=base, imm=imm, index=index, scale=scale_lookup[scale], disp=disp)
                return (size, oper)
    ...
    def disasm(self, bytez, offset, va):
        ...
        for i in operand_range:
            ...
            if addrmeth == 0:
                osize = 0
                oper = self.ameth_0(
                    operflags, opdesc[5+i], tsize, all_prefixes)
            else:
    ...
    def ameth_x(self, bytez, offset, tsize, prefixes, operflags):
        ...
        return (0, i386RegMemOper(e_i386_regs.REG_ESI, tsize))
