...
def p_load_reg_off(opval, va):
    ...
    olist = (
        ArmRegOper(Rd, va=va),
        ArmScaledOffsetOper(Rn, Rm, shtype, shval, va,
                            pubwl),  # u=-/+, b=word/byte
    )
...
def p_load_mult(opval, va):
    ...
    # store bits for decoding whether to dec/inc before/after between ldr/str.  IF_DA tells the repr to print the the DAIB extension after the conditional.  right shift necessary to clear lower three bits, and align us with IF_DAIB_SHFT
...
def p_coproc_dp(opval, va):
    ...
    # FIXME: CDP2 (cond = 0b1111) also needs handling.
    return (opcode, mnem, olist, 0)
...
def p_uncond(opval, va):
    ...
    if optop == 0:
        ...
        if (opval & 0xf570f000) == 0xf550f000:
            ...
            mnem = "pld"
            # what the freak am i supposed to do with "i"???
            I = (opval >> 25) & 1
            Rn = (opval >> 16) & 0xf
            U = (opval >> 23) & 1
            ...
            if I:
                ...
                shtype = (opval >> 5) & 3
                shval = (opval >> 7) & 0x1f
                olist = (ArmScaledOffsetOper(
                    Rn, Rm, shtype, shval, va, pubwl), )
            return (opcode, mnem, olist, 0)
        else:
            raise envi.InvalidInstruction(
...
s_0_table = (
    ...
    (0b00000001100100000000000000010000,
     0b00000001000000000000000000000000, IENC_MISC),
    (0b00000000000000000000000000010000,
     0b00000000000000000000000000000000, IENC_DP_IMM_SHIFT),
    (0b00000001100100000000000010010000,
     0b00000001000000000000000000010000, IENC_MISC1),
    (0b00000001000000000000000011110000,
     0b00000000000000000000000010010000, IENC_MULT),
    (0b00000001001000000000000010010000,
     0b00000001001000000000000010010000, IENC_EXTRA_LOAD),
    (0b00000000000000000000000010010000,
     0b00000000000000000000000010010000, IENC_EXTRA_LOAD),
    (0b00000000000000000000000010010000,
     0b00000000000000000000000000010000, IENC_DP_REG_SHIFT),
    (0, 0, IENC_UNDEF),  # catch-all
)

s_1_table = (
    (0b00001111101100000000000000000000,
     0b00000011001000000000000000000000, IENC_MOV_IMM_STAT),
    (0b00001111111100000000000000000000,
     0b00000011000000000000000000000000, IENC_DP_MOVW),
    (0b00001111111100000000000000000000,
     0b00000010100000000000000000000000, IENC_DP_MOVT),
    (0b00001111111100000000000000000000,
     0b00000010010000000000000000000000, IENC_DP_MSR_IMM),
    (0b00001111111100000000000000000000,
     0b00000010110000000000000000000000, IENC_DP_MSR_IMM),
    (0b00001110000000000000000000000000,
     0b00000010000000000000000000000000, IENC_DP_IMM),
    (0, 0, IENC_UNDEF),
)

s_3_table = (
    (0b00000001111100000000000011110000,
     0b00000001111100000000000011110000, IENC_ARCH_UNDEF),
    (0b00000000000000000000000000010000,
     0b00000000000000000000000000010000, IENC_MEDIA),
    (0, 0, IENC_LOAD_REG_OFF),
)

s_6_table = (
    (0b00001111111000000000000000000000,
     0b00001100010000000000000000000000, IENC_COPROC_RREG_XFER),
    (0b00001110000000000000000000000000,
     0b00001100000000000000000000000000, IENC_COPROC_LOAD),
)

s_7_table = (
    (0b00000001000000000000000000000000,
     0b00000001000000000000000000000000, IENC_SWINT),
    (0b00000001000000000000000000010000,
     0b00000000000000000000000000010000, IENC_COPROC_REG_XFER),
    (0, 0, IENC_COPROC_DP),
)
    ...
    def getBranches(self, emu=None):
        ...
        if self.opcode in (INS_B, INS_BX, INS_BL, INS_BLX, INS_BCC):
            ...
            if self.opcode in (INS_BLX, INS_BX):
                if operval & 3:
                    flags |= e_const.ARCH_THUMB16
                else:
                    flags |= e_const.ARCH_ARMV7
...
class ArmScaledOffsetOper(ArmOperand):
    ...
    def getOperAddr(self, op, emu=None):
        ...
        addval = shifters[self.shtype](
            emu.getRegister(self.offset_reg), self.shval)
        # if U==0, subtract
...
class ArmImmOffsetOper(ArmOperand):
    ...
    def render(self, mcanv, op, idx):
        ...
        if self.base_reg == REG_PC:
            ...
            # only works without an emulator because we've already verified base_reg is PC
            addr = self.getOperAddr(op, mcanv.mem)
...
fields = (None, 'c', 'x', 'cx', 's', 'cs', 'xs', 'cxs',  'f',
          'fc', 'fx', 'fcx', 'fs', 'fcs', 'fxs', 'fcxs')
...
class ArmPSRFlagsOper(ArmOperand):
    ...
    def getOperValue(self, op, emu=None):
        if emu == None:
            return None
        raise Exception(
            "FIXME: Implement ArmPSRFlagsOper.getOperValue() (does it want to be a bitmask? or the actual value according to the PSR?)")
        return None  # FIXME
...
class ArmDisasm:
    ...
    def disasm(self, bytez, offset, va):
        ...
        # FIXME: this could backfire if COND_EXTENDED...
        if cond == COND_AL:
