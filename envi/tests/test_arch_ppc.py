...
class PpcInstructionSet(unittest.TestCase):
    ...
    def _do_CR_XER(self, op, emu, r1, r2, r3, cr, xer, expr3, expcr, expxer):
        ...
        self.assertEqual((repr(op), hex(r1), hex(r2), hex(r3), hex(cr), hex(xer), hex(newr3), hex(newcr), hex(newxer)), \
                (repr(op), hex(r1), hex(r2), hex(r3), hex(cr), hex(xer), hex(expr3), hex(expcr), hex(expxer)))
