...
class LockStepper:
    ...
    def cmpregs(self, emuop, traceop):
        ...
        for regname, regval in emuregs.items():
            ...
            if traceval != regval:
                raise Exception('LockStep: emu 0x%.8x %s %s=0x%.8x | trace 0x%.8x %s %s=0x%.8x' % (
                    emuop.va, repr(emuop), regname, regval, traceop.va, repr(traceop), regname, traceval))
    ...
    def cmppages(self, emuop, traceop):
        for va, bytez in self.memcache.getDirtyPages():
            ...
            if diffs:
                diffstr = ','.join(['0x%.8x: %d' % (va+offset, size)
                                    for offset, size in diffs])
                raise Exception('LockStep: emu 0x%.8x %s | trace 0x%.8x %s | DIFFS: %s' % (
                    emuop.va, repr(emuop), traceop.va, repr(traceop), diffstr))
