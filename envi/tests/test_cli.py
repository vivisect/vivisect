...
class EnviCliTest(unittest.TestCase):
    def test_do_cmds(self):
        ...
        # do_alias
        ecli.do_alias('')
        aliasout = ecli.canvas.strval
        self.assertIn('Runtime Aliases (not saved):', aliasout)
        self.assertIn('Configured Aliases:', aliasout)
        ecli.canvas.clearCanvas()

        # do_binstr
        ecli.do_binstr('47145')
        aliasout = ecli.canvas.strval
        self.assertIn('0x0000b829 (47145) 1011100000101001', aliasout)
        ecli.canvas.clearCanvas()
        ...
        ecli.do_maps('')
        aliasout = ecli.canvas.strval
        self.assertIn('0x41410000', aliasout)
        ecli.canvas.clearCanvas()
