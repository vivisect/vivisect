...
class VStructBuilder:
    ...
    def buildVStruct(self, vsname):
        ...
        if final_len < vssize:
            setattr(vs, '_pad%.4x' %
                    vssize, vs_prim.v_bytes(size=(vssize-final_len)))
    ...
    def genVStructPyCode(self):
        ...
        for vsname, vsize, vskids in self._vs_defs.values():
            ...
            for fname, foffset, fsize, ftypename, fflags, fcount in vskids:
                ...
                if fcount is not None:
                    fconst = 'vstruct.VArray([ %s for i in xrange(%d) ])' % (
                        fconst, fcount)
                    fsize *= fcount
...
# TODO(rakuyo): move this into a general tools directory
if __name__ == '__main__':
    ...
    with PE.peFromFileName(sys.argv[1]) as p:
