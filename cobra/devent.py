...
class CobraEventCore:
    ...
    def cullAbandonedChannels(self, abtime):
        [self.finiEventChannel(q.chanid)
         for q in self._ce_chans if q.abandoned(abtime)]
    ...
    def finiEventChannels(self):
        ...
        [self.finiEventChannel(chanid)
         for chanid in self._ce_chanlookup.keys()]
    ...
    def _fireEvent(self, event, einfo, upstream=True, skip=None, chans=None):
        ...
        if self._ce_ecastsock:
            self._ce_ecastsock.sendto(json.dumps(
                etup), (self._ce_mcasthost, self._ce_mcastport))
    ...
    def addEventUpstream(self, evtcore, qmax=0, timeout=5):
        ...
        while True:
            ...
            try:
                [self.fireEvent(event, einfo, upstream=False)
                 for (event, einfo) in events]
            except Exception as e:
    ...
    def addEventCallback(self, callback, qmax=0, firethread=True):
        ...
        if firethread:
            thr = threading.Thread(
                target=self.addEventCallback, args=(callback, qmax, False))
            thr.setDaemon(True)
            thr.start()
            return
