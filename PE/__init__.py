...
# Code integrity checks are forced. If you set this flag and a section contains only uninitialized
# data, set the PointerToRawData member of IMAGE_SECTION_HEADER for that section to zero;
...
class PE(object):
    ...
    def parseResources(self):
        ...
        # RP BUG FIX - Binaries can have a .rsrc section it doesn't mean that the .rsrc section
        # contains the resource data we think it does
        ...
        while len(rsrc_todo):
            ...
            for i in range(totcount):
                ...
                if dirent.OffsetToData & 0x80000000:
                    ...
                    subdata = self.readStructAtRva(dresc.VirtualAddress + dirent.OffsetToData,
                                                   'pe.IMAGE_RESOURCE_DATA_ENTRY')
    ...
    def parseSections(self):
        ...
        off += self.IMAGE_NT_HEADERS.OptionalHeader.NumberOfRvaAndSizes * \
            len(vstruct.getStructure("pe.IMAGE_DATA_DIRECTORY"))
    ...
    def parseLoadConfig(self):
        ...
        if self.checkRva(rva, size=cdir.Size):
            self.IMAGE_LOAD_CONFIG = self.readStructAtRva(
                rva, "pe.IMAGE_LOAD_CONFIG_DIRECTORY")
            ...
            while True:
                ...
                if ibn_rva & self.high_bit_mask:
                    funcname = ordlookup.ordLookup(
                        libname, ibn_rva & 0x7fffffff)

                elif not self.checkRva(ibn_rva):
                    break

                else:
                    # RP BUG FIX - we can't use this API on this call because we can have
                    # binaries that put their import table right at the end of the file,
                    # statically saying the imported function name is 128 will cause use to
                    # potentially over run our read and traceback...
    ...
    def parseRelocations(self):
        ...
        while relbytes:
            ...
            # RP BUG FIX - sometimes the chunksize is invalid we do a quick check to
    ...
    def parseExports(self):
        ...
        self.IMAGE_EXPORT_DIRECTORY = self.readStructAtOffset(
            poff, "pe.IMAGE_EXPORT_DIRECTORY")
    ...
    def getSignCertInfo(self):
        ...
        substrate = sig.pkcs7
        contentInfo, rest = pyasn1.codec.der.decoder.decode(
            substrate, asn1Spec=pyasn1_modules.rfc2315.ContentInfo())
