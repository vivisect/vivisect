...
def secname(value):
    if value[0] == b'/':
        return string_table_entry(value[1:].rstrip(b'\0'))
    else:
        return '%s'
...
def read(format):
    read = Input.read(struct.calcsize(format))
    return struct.unpack(format, read)
...
def write_value(fmt, value):
    ...
    if isinstance(fmt, str):
        write(fmt % value)
    elif isinstance(fmt, types.FunctionType):
        write_value(fmt(value), value)
    elif isinstance(fmt, tuple):
        Fields['this'] = value
        handle_element(fmt)
    elif isinstance(fmt, type(None)):
        pass
    else:
...
def handle_flags(entry):
    ...
    for entry in definitions:
        ...
        if len(entry) == 3:
            ...
            if selection in map:
                write("\n%s" % map[selection])
                flags_set.append(map[selection])
            else:
                write("\n%s <%d>" % (name, selection))
        elif len(entry) == 2:
            if value & mask != 0:
...
def handle_blob(entry):
    ...
    value = Input.read(read_value(length))
...
def cleanup_symbol_name(name):
    parts = struct.unpack("<2L", name)
    if parts[0] == 0:
        x = string_table_entry(parts[1])
        cleaned = handle_element(x)
    else:
        cleaned = name.rstrip(b'\0')
...
def parse_file(file_obj):
    global Input
    Input = file_obj
    parsed = handle_element(file)
    for symbol in parsed['Symbols'].values():
        symbol['Name'] = cleanup_symbol_name(symbol['Name'])
    for section in parsed['Sections']:
        section['Name'] = section['Name'].rstrip(b'\0')
        if section['Relocations'] is None:
            section['Relocations'] = list()
        for reloc in section['Relocations']:
            reloc['SymbolName'] = cleanup_symbol_name(reloc['SymbolName'])
    return parsed
